 ...
 */
void print_multi_plan_result(const std::string& label, const TrajResult& res, const std::string& chain_name,
                             GalbotMotion& motion) {
  ...
  if (status == MotionStatus::SUCCESS) {
    if (traj_map.count(chain_name) && !traj_map[chain_name].empty()) {
      std::cout << "✅ Multi-point planning successful: Total trajectory points = " << traj_map[chain_name].size()
                << std::endl;
    } else {
      std::cout << "⚠️ Status SUCCESS but trajectory is empty, possibly due to target coinciding with current pose."
                << std::endl;
    }
  } else {
...
int main() {
  ...
  try {
    ...
    MotionStatus status = motion.add_obstacle(obstacle_id, obj_type, obj_pose, obj_size, target_frame);
  ...
  try {
    ...
    std::vector<std::vector<double>> waypoint_poses = {{0.1267, 0.2342, 0.7356, 0.0220, 0.0127, 0.0343, 0.9991},
    ...
    auto res = motion.motion_plan_multi_waypoints(target_pose_state, waypoint_poses,
                                                  nullptr,  // start
  ...
  try {
    ...
    // Construct waypoints (3 intermediate poses)
    std::vector<std::vector<double>> waypoints = {{0.1267, 0.2342, 0.7356, 0.0220, 0.0127, 0.0343, 0.9991},
    ...
    auto res = motion.motion_plan_multi_waypoints(target_joint, waypoints, nullptr, nullptr, false, params);
