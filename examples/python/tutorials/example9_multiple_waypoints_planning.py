...
try:
    ...
    from galbot_sdk.g1 import GalbotRobot, GalbotMotion, GalbotNavigation, SensorType, JointGroup, ControlStatus, Trajectory, TrajectoryPoint,JointCommand
except ImportError:
...
def main():
    ...
    try:
        ...
        try:
            ...
            if status == gm.MotionStatus.SUCCESS:
                if traj != {}:
                    print(f"✅ Multi-path Point Planning in Cartesian Space (PoseState Target) Success: 轨迹点数={len(traj[target_pose_state.chain_name])}")
                else:
                    print(f"⚠️ Multi-path Point Planning in Cartesian Space (PoseState Target) Success: 轨迹为空，可能已到达，检查目标值与当前状态是否一致或在误差范围内")
            else:
        ...
        try:
            ...
            if status == gm.MotionStatus.SUCCESS:
                if traj != {}:
                    print(f"✅ Multi-path Point Planning in Joint Space (JointStates Target) Success: 轨迹点数={len(traj[target_joint.chain_name])}")
                else:
                    print(f"⚠️ Multi-path Point Planning in Joint Space (JointStates Target) Success: 轨迹为空，可能已到达，检查目标值与当前状态是否一致或在误差范围内")
            else:
